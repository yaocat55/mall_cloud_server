...
public class AdminProductManagerController {
    ...
    public ResponsePageEntity<?> searchCategoryPage(@RequestBody CategoryConditionDTO c) {
        return categoryFeignClient.searchByPage(c);
    }
    ...
    public List<?> getCategoryTree(@RequestBody Map c) {
        return categoryFeignClient.searchByTree(c);
    }

    @Operation(summary = "分页查询品牌", description = "多条件分页查询商品品牌列表", security = @SecurityRequirement(name = "Bearer Token"))
    @PostMapping("/brand/page")
    public ResponsePageEntity<?> searchBrandPage(@RequestBody BrandConditionDTO c) {
        return brandFeignClient.searchByPage(c);
    }

    @Operation(summary = "分页查询单位", description = "多条件分页查询商品单位列表", security = @SecurityRequirement(name = "Bearer Token"))
    @PostMapping("/unit/page")
    public ResponsePageEntity<?> searchUnitPage(@RequestBody UnitConditionDTO c) {
        return unitFeignClient.searchByPage(c);
    }
}
