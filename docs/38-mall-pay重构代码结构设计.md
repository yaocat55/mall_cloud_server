...
支付是**状态机 + 账目**系统，每一个状态流转都是**持久化事实**，缓存策略按层区分：

- **支付状态（待支付→已支付）不能有缓存层**——缓存里是"已支付"但 DB 还是"待支付"，对账立刻发现短款，资金风险不可接受
- **渠道配置（`pay_channel_config`）读多写极少，使用 Caffeine 本地缓存**——TTL 10min，admin 修改时即时淘汰。渠道密钥变更极低频（证书一年一换），Caffeine 本地缓存避免了每次 prepay 查库和反复 new DefaultAlipayClient 的开销。AlipayClient 实例本身也由 AlipayClientFactory 工厂以 Caffeine 缓存（maxSize=16、TTL 30min），一个渠道一个实例复用，不再每次请求重建
- **幂等（回调防重放）用 Redis 存 nonce 是唯一的例外**——它不是业务状态缓存，是短期去重标记（TTL 5 分钟），不影响账目一致性
