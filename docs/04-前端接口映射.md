...
| 前端页面 | 调用 URL | 方法 | 后端 Class | 后端方法 | @RequestMapping |
|---------|----------|------|-----------|----------|----------------|
| 收银台-渠道列表 | `/pay/v1/mobile/pay/channels` | GET | PayController | `channels()` | `@RequestMapping("/v1/mobile/pay")` |
| 收银台-创建支付 | `/pay/v1/mobile/pay/create` | POST | PayController | `create()` | 同上 |
| 收银台-查询支付 | `/pay/v1/mobile/pay/query` | POST | PayController | `query()` | 同上 |
| 收银台-关闭支付 | `/pay/v1/mobile/pay/close` | POST | PayController | `close()` | 同上 |
| MOCK 模拟回调 | `/pay/v1/notify/mock?merchantOrderNo=xxx` | POST | MockNotifyController | `handleMockNotify()` | `@RequestMapping("/v1/notify")`（仅 dev） |

> ⚠️ 旧版 `/pay/v1/mobile/pay/mockPay` 已废弃。MOCK 渠道（仅 dev/test 启用 `mall.pay.mock.enabled=true`）
> 的支付链路改为：`create(MOCK)` → 前端调用 `/v1/notify/mock?merchantOrderNo=xxx` 模拟回调 → 支付状态扭转 + MQ 通知。
...
### 4.2 mall-admin-api（端口 8030，前端直连）

**✅ 前端直连（经网关 `/api/admin-api/**`）：**
- 用户/角色/菜单/部门/岗位 CRUD → `/api/admin-api/v1/auth/**`
- 登录/登出/用户信息/菜单树 → `/api/admin-api/v1/auth/web/user/**`

> ⚠️ 原 BFF 聚合接口 `edit-data`（用户信息+角色+部门树、商品详情+分类树）已随 admin-bff 拆除，前端自行聚合。
